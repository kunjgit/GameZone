import { BASE_POSITIONS, HOME_ENTRANCE, HOME_POSITIONS, PLAYERS, START_POSITIONS, STATE, TURNING_POINTS } from '../common/constant4player.js';
import { UI } from './UI4P.js';
...
export class Ludo {
    ...
    onPieceClick(event) {
        const target = (event.target.classList.contains('player-piece-img')) ? event.target.parentElement : event.target;
    ...
    checkForKill(player, piece) {
        const currentPosition = this.currentPositions[player][piece];
        const opponentList = ['P1', 'P2', 'P3', 'P4'];
        const index = opponentList.indexOf(player);
